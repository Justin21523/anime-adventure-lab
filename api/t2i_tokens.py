...
Adapted from charaforge-T2I-Lab/api/t2i_tokens.py for anime-adventure-lab.

Replaced `get_cache_paths()` with plain `~/.anime-adventure-lab` so this file
doesn't add a hard dependency on `core.config` config-specific functions.
"""
...
def _secret_path() -> Path:
    """Location for the persistent signing secret."""
    data_dir = Path(os.getenv("DATA_DIR", Path.home() / ".anime-adventure-lab"))
    return data_dir / "auth" / "signing_secret.txt"
...
def signing_secret() -> bytes:
    """Return the signing secret, preferring JWT_SECRET / API_SIGNING_SECRET env vars."""
